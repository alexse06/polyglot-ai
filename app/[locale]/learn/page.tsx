...
import { CheckCircle, Lock, Play, Star, Sparkles } from 'lucide-react';
...
export default async function LearnPage() {
    ...
    const isEnglish = user?.learningLanguage === 'EN';

    return (
        <div className="min-h-screen text-white pb-20">
            <header className="fixed top-0 w-full z-10 glass-panel backdrop-blur px-4 py-3 flex justify-between items-center rounded-b-2xl mb-8">
                <div className="flex items-center gap-2">
                    ...
                    <h1 className="font-bold">Leçons</h1>
                </div>
            </header>

            <main className="pt-24 max-w-2xl mx-auto px-4 space-y-8">
                <div className="text-center mb-8">
                    <h2 className="text-3xl font-bold bg-gradient-to-r from-yellow-400 to-orange-500 bg-clip-text text-transparent animate-pulse-glow inline-block">
                        {isEnglish ? 'Parcours A1 (English)' : 'Parcours A1 (Espagnol)'}
                    </h2>
                    <p className="text-gray-400 mt-2">
                        {isEnglish ? "Maîtrisez les bases de l'anglais" : "Maîtrisez les bases de l'espagnol"}
                    </p>
                </div>
