...
async function seedLessons(existingTitles: string[] = [], language: string = "ES") {
    ...
    const curriculumEN = [
        ...
        { title: "Au restaurant", topic: "Ordering food, polite requests (would like)", level: "A1" },
        { title: "Voyage", topic: "Travel directions and transport", level: "A1" },
        { title: "La Famille", topic: "Family members and possessives", level: "A1" },
        { title: "Les Nombres", topic: "Numbers, time, and prices", level: "A1" },
        { title: "Ma Routine", topic: "Present Simple and daily habits", level: "A1" },
        { title: "Au marché", topic: "Food items and Countable/Uncountable", level: "A1" },
        { title: "Les Hobbies", topic: "Likes and Dislikes (gerunds)", level: "A1" },
        // A2
        { title: "Le Passé (Simple Past)", topic: "Talking about finished actions", level: "A2" },
        { title: "Futur (Will/Going to)", topic: "Future plans and predictions", level: "A2" },
        { title: "La Maison", topic: "Home description and prepositions", level: "A2" },
        { title: "Chez le Médecin", topic: "Health problems and advice (should)", level: "A2" },
        { title: "La Météo", topic: "Weather vocabulary", level: "A2" },
        { title: "Vêtements", topic: "Shopping for clothes", level: "A2" },
        { title: "Sentiments", topic: "Feelings and Adjectives", level: "A2" }
    ];

    const curriculum = language === "EN" ? curriculumEN : curriculumES;
...
export async function generateNextLessonAction() {
    ...
    if (!userWithProgress) throw new Error("User not found");
