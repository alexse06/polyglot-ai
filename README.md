...
|-------|---------|-------------|
| **`/`** | Landing Page | Public showcase with features, hero section, and CTA. |
| **`/dashboard`** | **Main Hub** | Central dashboard showing daily quests, XP, streaks, and access to all modules. |
| **`/live`** | **Live Coach** | **(Gemini 2.5)** Real-time voice tutor with 3 personas (Tutor, Barista, Doctor). Includes Immersion Mode. |
| **`/career`** | Career Coach | **(Beta)** Job interview simulator. AI Recruiter asks questions based on your CV profile. |
| **`/pronounce`** | Vocal Coach | Speech analysis tool. Read sentences and get feedback on pronunciation accuracy. |
| **`/chat`** | AI Chat | Free-form text conversation with corrections. Good for writing practice. |
| **`/scenarios`** | Roleplay | Specific missions (e.g. "Buy a train ticket") with success criteria. |
| **`/learn`** | Lessons | Structured grammar and vocabulary lessons generated by AI. |
| **`/flashcards`** | Flashcards | Spaced repetition system (SRS) for vocabulary retention. |
| **`/characters`** | Writing | Learn specific scripts (Kanji, Cyrillic, Hanzi) for applicable languages. |
| **`/profile`** | User Profile | Manage settings, language preferences, and view detailed progress history. |
| **`/admin`** | Admin Panel | User management and content oversight (Protected). |
