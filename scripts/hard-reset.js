...
async function main() {
    ...
    console.log("Deleting UserScenarioProgress...");
    ...
    console.log("--- Reseeding Default Scenarios ---");
    ...
    // Seed BOTH languages
    for (let i = 0; i < scenariosES.length; i++) {
        await prisma.scenario.create({
            data: { ...scenariosES[i], language: "ES", order: i }
        });
        console.log(`Created ES: ${scenariosES[i].title}`);
    }

    for (let i = 0; i < scenariosEN.length; i++) {
        await prisma.scenario.create({
            data: { ...scenariosEN[i], language: "EN", order: i }
        });
        console.log(`Created EN: ${scenariosEN[i].title}`);
    }
